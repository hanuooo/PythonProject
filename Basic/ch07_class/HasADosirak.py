...
class Bag:
    ...
    def showList(self):
        print('# 도시락 목록 확인')
        for item in self.dosirakList: # item = dosirak 1개
            print('이름 : %s, 단가 : %d원 ' % (item.name, item.price))
            print('반찬 목록 : ', end='')
...
dosirak02 = Dosirak('매화 도시락', 10000, '고급어묵', '김치', '연근 조림', '단호박 샐러드')
mybag.open()
mybag.put(dosirak02)
mybag.close()