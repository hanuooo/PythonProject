...
print(f'\n교사 학습법 : 문제지와 답지를 같이 나눠주고, 분석을 예측하는 기법')
...
print(f'\n중복되는 행 개수 : {df.duplicated().sum()}개 입니다.') # {sum(df.duplicated())} 동일
...
print('승선 위치 컬럼의 결측치(nan)는 빈도수가 갖아 많은 항목으로 대체합니다.')
print('항목별 빈도 수')
...
print('# 종속 변수 : 독립 변수에 의하여 영향을 받는 변수 (output 데이터 : 답지 )')
print('# 특정 변수와 종속 변수 분리')
...
# fit() 함수로 해당 모델을 훈련시켜 주는 함수입니다.
# train : 학습용 , test : 테스트용
model.fit(x_train, y_train) # 내가 공부할 시험지와 답지

print('\npredict 함수는 가장 높은 확률의 클래스 값( 0 또는 1 )을 반환해 줍니다.')
prediction = model.predict(x_test) # x_test 답지없이 시험본 나의 문제지

print('# 예측 값(모의고사에서 내가 작성한 답지)')
print(prediction[0:10])

print('\n# 실제 정답 데이터(모의고사의 진짜 답지)')
print(y_test.values[0:10])

print('\n# 예측값과 실제값을 하나의 DataFrame으로 만들기')
result_df = pd.DataFrame({
    'actual': y_test.values, # 실제 정답
    'predicted': prediction, # 예측 값
    'correct': y_test.values == prediction # 맞았으면 True , 틀리면 False
})
...
# 정답 양성,음성 항목 : ( TN , TP ) , 문제가 있는 항목 ( FN , FP )
...
prediction_probability = model.predict_proba(x_test) # probability : 확률
