...
# Found 20 image belonging to 2 classes.
# 2개의 클래스에 속해 있는 이미지 20개가 발견되었습니다.

print(f'type(idg): {type(iters)}') # DirectoryIterator 객체
print(f'한번에 실행할 사이즈 : {iters.batch_size}') # batch_size 속성과 연관 있음
print(f'이미지 형상 정보 : {iters.image_shape}') # target_size 와 color_mode와 연관 있음
print(f'클래스 레이블과 색인 정보 : {iters.class_indices}')

print('next() 함수는 반복자(iterator) 객체인 lters에서 다음 항목을 읽어 주는 파이썬내장 함수입니다.')
